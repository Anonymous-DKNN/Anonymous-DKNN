...
from codes import denoise, featurebank
import torch
...
def main():
    ...
    dpath = f'features/{dataset_name}/denoisescores'

    if args.mode == 'app':
        ret = denoise.AppAErecon(dataset_name, uvadmode).infer()
        fpath = f'{dpath}/{uvadmode}_aerecon_flat.npy'
    elif args.mode == 'mot':
        tr_f = featurebank.get(dataset_name, 'mot', 'train', uvadmode=uvadmode).astype(np.float32)
        ret = denoise.fGMM(dataset_name, uvadmode, tr_f, args.gmm_n).infer(tr_f)
