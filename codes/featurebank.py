...
def get(dataset_name, feature_name, mode, uvadmode='partial'):
    ...
    assert uvadmode in ['partial', 'merge', 'supervised']
    ...
    elif uvadmode == 'supervised':
        return _get_supervisedmode(dataset_name, feature_name, mode)
    else:
        raise ValueError()
...
def _get_supervisedmode(dataset_name, feature_name, mode):
    dpath = f'features/{dataset_name}'
    if mode == 'train':
        fpath1 = f'{dpath}/train/{feature_name}.npy'
