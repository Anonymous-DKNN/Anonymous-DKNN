...
class KNNMultiGrader(ABCGrader):
    def __init__(self, tr_x, K=1, key=''):
        super().__init__(key)
        self.K = K
        self.tr_x = tr_x
...
class GMMGrader(ABCGrader):
    def __init__(self, tr_x, n_componnents=2, key=''):
        super().__init__(key)
        self.tr_x = tr_x
        self.est = GaussianMixture(n_components=n_componnents, random_state=0).fit(tr_x)

    def grade_flat(self, te_x):  # [M, D]
        return -self.est.score_samples(te_x)
